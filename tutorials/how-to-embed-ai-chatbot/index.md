...
* popup - _"Popup text"_ that's shown 5 seconds after the AI chatbot loads, to make it slightly more _"intrusive"_ and visible. Check our website for an example
...
* extra - Additional extra parameters, which can be anything, and are typically used for complex integrations where _"instance information"_ is required on a _"per chatbot window"_ basis
...
RTL implies _"right to left"_ and is the way Hebrew, Arabic, and Farsi are written. By setting this property to `true`, the chatbot will add the required HTML to render these languages correctly. If you know for a fact that all your users will be writing in Arabic, Farsi, or Hebrew (or another language which is RTL-based), you can set this setting to true.
...
The reason is that it's the LLM's job to actually generate follow up questions that are relevant to the current context and previous questions. However, you still have to make sure the `follow_up` query parameter is set to `true` to allow for the frontend to correctly render these as follow up questions.
