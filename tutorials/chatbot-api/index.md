...
* type - This is the machine learning model you wish to use - the argument is named "type" since that is what the API calls a model. Mandatory.
...
* recaptcha_response - If your machine learning model is configured to use Google reCAPTCHA, then this is the reCAPTCHA token. If not, it's expected to be a Magic PoW token. Notice, we advise all our clients to use Magic PoW CAPTCHA since it's much smaller in size, and also _"good enough"_ for most use cases. Read more about [Magic's PoW CAPTCHA here](/tutorials/magic-captcha-outperforming-google-2567-times/). Mandatory argument.
...
```json
{
  "prompt": "How's the weather in Larnaca?",
...
If you set the above `stream` argument to true, the above endpoint will _not_ wait for the LLM's response, but rather stream the response one token at the time to a SignalR channel with its name being the same as your `session` value, and the endpoint will return immediately long before the response is ready. This implies you first have to connect to the cloudlet using SignalR from your client, for then to listen to SignalR events using your unique `session` ID.

This argument should as a general rule of thumb _almost always be set to true_, to allow for streaming the response. The reason for this is that the CDN we're using will shut down the HTTP connection if the cloudlet takes more than 60 seconds to answer. And with multiple questions, maybe analysing files too, and also possibly multiple function invocations within the cloudlet, the LLM might easily end up in a situation where it requires more than 60 seconds to provide an answer.

This implies the steps for initiating a chat session with your cloudlet become as follows;
...
The above assumes you've got a `this.socket` property on your object, being the actual socket connection where the cloudlet should transmit streamed messages. In addition it assumes you've got a `this.response` property, intended to contain the complete response from the LLM. Notice, only the delta parts are transmitted as `obj.message`, and not the entire message - So it's up to your client-side code to keep track of messages previously sent - Which is why the above `this.response += obj.message` works.

Streaming is the most difficult part to get correct when implementing your own client-side code, but it's crucial to get these parts right, and it's also highly rewarding to see the response from the LLM being streamed back to the client, one token at the time.
